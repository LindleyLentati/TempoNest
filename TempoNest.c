...
// 				if(k == 2 || k == 3){TempoPriors[paramsfitted][1] *= 100;}
				...
				//printf("Prefit: %s %.25Lg %.25Lg %g \n", psr[0].param[p].shortlabel[k],psr[0].param[p].prefit[k],psr[0].param[p].err[k],psr->fitChisq);
				paramsfitted++;
				getdistlabel++;
	...
	for(int i=0;i<=psr[0].nJumps;i++){
		if(psr[0].fitJump[i] == 1){
			...
			int nread=sscanf(psr[p].jumpStr[i],"%s %s %s %s %s",str1,str2,str3,str4,str5);
...
// 	return 0;
	...
	if(doLinearFit != 1){
		...
		if(doJumpMargin != 0 || doTimeMargin != 0 ){
			...
			if(doTimeMargin==2)numToMargin += fitcount-1;
	...
	// 			TNGM[i]=new double[psr[0].nobs-numToMargin];
			...
			getMarginDMatrix(((MNStruct *)context)->pulse, fitcount, numFitJumps, numToMargin, TempoFitNums, TempoJumpNums, Dpriors, doJumpMargin, doTimeMargin, DMargin, 1);
